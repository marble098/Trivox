...
internal object IcmpProbe {
    ...
    internal fun parseOutput(
        output: String,
        verifiedReplyAddresses: Set<String>
    ): Parsed {
        val expected = verifiedReplyAddresses
            .map(::normalizeAddress)
            .filter(String::isNotBlank)
            .toSet()
        ...
        output.lineSequence()
            ...
            .forEach { line ->
                if (!line.contains("icmp_seq", ignoreCase = true)) {
                    return@forEach
                }
                sawReply = true
                ...
                val address = replyAddressCandidates(line)
                    .asSequence()
                    .map(::normalizeAddress)
                    .firstOrNull {
                        it.isNotBlank() && it in expected
                    }
                ...
                val raw = timeMatch.groupValues[2]
                    .toDoubleOrNull()
                    ?: return@forEach
                val value =
                    if (timeMatch.groupValues[1] == "<") {
                        raw / 2.0
                    } else {
                        raw
                    }
        ...
        return Parsed(
            samplesMs = samples,
            replyAddress = acceptedAddress,
            sawReply = sawReply,
            sawUnexpectedReply = sawUnexpected
        )
    }

    private fun replyAddressCandidates(
        line: String
    ): List<String> {
        val parenthesized =
            PAREN_REPLY_ADDRESS.find(line)
                ?.groupValues
                ?.getOrNull(1)
                ?.trim()
    ...
    private val PAREN_REPLY_ADDRESS = Regex(
        """(?i)\bfrom\s+\S+\s+\(([^)]+)\)"""
    )
    private val PLAIN_REPLY_ADDRESS = Regex(
        """(?i)\bfrom\s+([^\s]+)"""
    )
    private val TIME_PATTERN = Regex(
        """(?i)\btime\s*([=<])\s*([0-9.]+)\s*ms"""
    )
