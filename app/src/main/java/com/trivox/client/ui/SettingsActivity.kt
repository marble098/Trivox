...
class SettingsActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val socks =
            findViewById<EditText>(R.id.socksPort)
        val http =
            findViewById<EditText>(R.id.httpPort)
        val mtu =
            findViewById<EditText>(R.id.mtuInput)
        val ipv6 =
            findViewById<CheckBox>(R.id.ipv6Check)
        val dns =
            findViewById<Spinner>(R.id.dnsMode)
        val custom =
            findViewById<EditText>(R.id.customDns)
        val network =
            findViewById<CheckBox>(R.id.reconnectNetwork)
        val boot =
            findViewById<CheckBox>(R.id.reconnectBoot)
        val blocking =
            findViewById<CheckBox>(R.id.blockingCheck)
        ...
        language.adapter = compactAdapter(languages)
        ...
        socks.setText(settings.socksPort.toString())
        http.setText(settings.httpPort.toString())
        mtu.setText(settings.mtu.toString())
        ipv6.isChecked = settings.ipv6
        custom.setText(settings.customDns.joinToString("\n"))
        network.isChecked = settings.reconnectOnNetworkChange
        boot.isChecked = settings.reconnectOnBoot
        ...
        findViewById<Button>(R.id.saveButton)
            .setOnClickListener {
                val socksPort =
                    socks.text.toString().toIntOrNull()
                val httpPort =
                    http.text.toString().toIntOrNull()
                val mtuValue =
                    mtu.text.toString().toIntOrNull()
                ...
                if (
                    socksPort == null ||
                    httpPort == null ||
                    !Validators.validPort(socksPort) ||
                    !Validators.validPort(httpPort) ||
                ...
                if (
                    modes[dns.selectedItemPosition] ==
                    DnsMode.CUSTOM &&
                    (
                        dnsValues.isEmpty() ||
                            !dnsValues.all(
