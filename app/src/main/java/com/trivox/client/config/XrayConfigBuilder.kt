...
object Validators {
    ...
    fun validateDns(value: String): Boolean {
        ...
        if (listOf("https://", "https+local://", "tcp://", "quic://").any(text::startsWith)) return true
...
object XrayConfigBuilder {
    private val privateNetworks = listOf("0.0.0.0/8","10.0.0.0/8","100.64.0.0/10","127.0.0.0/8","169.254.0.0/16","172.16.0.0/12","192.0.0.0/24","192.0.2.0/24","192.88.99.0/24","192.168.0.0/16","198.18.0.0/15","198.51.100.0/24","203.0.113.0/24","224.0.0.0/4","240.0.0.0/4","::/128","::1/128","2001:db8::/32","fc00::/7","fe80::/10","ff00::/8")

    fun build(profile: ConfigProfile, settings: AppSettings, mode: ConnectionMode, tunFd: Int? = null, errorLogPath: String? = null): String {
        settings.normalize()
    ...
    private fun buildInbounds(settings: AppSettings, mode: ConnectionMode): JSONArray {
        val result = JSONArray()
        if (mode == ConnectionMode.VPN) result.put(tunInbound(settings))
        if (mode == ConnectionMode.PROXY || settings.localProxyInVpn) result.put(mixedInbound(settings))
        return result
    }

    private fun buildOutbounds(profile: ConfigProfile): JSONArray {
        val chain = ProxyChainCodec.decode(profile.outboundJson)
        val result = JSONArray()
        if (chain == null) {
            result.put(normalizeOutbound(JSONObject(profile.outboundJson)).put("tag", "proxy"))
        } else {
            val bridge = normalizeOutbound(JSONObject(chain.bridge.toString())).put("tag", "chain-bridge")
            val exit = normalizeOutbound(JSONObject(chain.exit.toString())).put("tag", "proxy")
                .put("proxySettings", JSONObject().put("tag", "chain-bridge").put("transportLayer", true))
            result.put(exit).put(bridge)
        }
        return result.put(JSONObject().put("protocol", "freedom").put("tag", "direct"))
    ...
    private fun mixedInbound(settings: AppSettings) = JSONObject().put("tag", "mixed-in").put("listen", "127.0.0.1")
        .put("port", settings.socksPort).put("protocol", "mixed")
