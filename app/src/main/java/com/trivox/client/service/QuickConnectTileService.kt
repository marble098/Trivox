...
class QuickConnectTileService : TileService() {
    ...
    override fun onClick() {
        ...
        unlockAndRun {
            ...
            val profileId = last?.profileId ?: repository.selectedId()
            ...
            if (mode == ConnectionMode.PROXY) {
                startConnection(ConnectionMode.PROXY, profile.id)
            } else if (VpnService.prepare(this) == null) {
                startConnection(ConnectionMode.VPN, profile.id)
            } else {
                launchQuickActivity()
            }
        }
    ...
    private fun updateTile() {
        ...
        if (Build.VERSION.SDK_INT >= 29) {
            tile.subtitle = when {
                active != null -> active.profileName.ifBlank {
                    getString(R.string.state_connected)
                }
                last != null -> last.profileName.ifBlank {
                    getString(R.string.quick_tile_ready)
                }
                else -> getString(R.string.quick_tile_no_profile)
            }
        }
        tile.updateTile()
    }
