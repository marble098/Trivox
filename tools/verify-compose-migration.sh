...
notification_support="app/src/main/java/com/trivox/client/service/NotificationSupport.kt"
notification_receiver="app/src/main/java/com/trivox/client/service/NotificationQuickSwitchReceiver.kt"
grep -Fq 'NotificationActionChoice.PAUSE' "$notification_support" || fail "configurable notification pause action missing"
grep -Fq 'NotificationQuickSwitchReceiver.PAUSE' "$notification_support" || fail "notification pause receiver wiring missing"
grep -Fq 'NotificationQuickSwitchReceiver.USE' "$notification_support" || fail "direct notification Use action missing"
grep -Fq 'NotificationQuickSwitchReceiver.NEXT_CONNECT' "$notification_support" || fail "direct notification Next action missing"
grep -Fq 'const val USE = "com.trivox.client.NOTIFY_SWITCH_USE_DIRECT"' "$notification_receiver" || fail "direct notification Use receiver missing"
grep -Fq 'const val PAUSE = "com.trivox.client.NOTIFY_PAUSE_DIRECT"' "$notification_receiver" || fail "direct notification Pause receiver missing"
